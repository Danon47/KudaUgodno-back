...
class HotelFilter(FilterSet):
    ...
    price_gte = NumberFilter(method="filter_by_price")
    price_lte = NumberFilter(method="filter_by_price")
    ...
    def filter_by_guests(self, queryset, name, value):
        ...
        try:
            guests = int(value)
            return queryset.filter(rooms__number_of_adults__gte=guests - F("rooms__number_of_children"))
    ...
    def filter_queryset(self, queryset):
        ...
        filters = Q(calendar_date__available_for_booking=True)
        if hasattr(self, "check_in_date"):
            filters &= Q(
                calendar_date__start_date__lte=self.check_in_date,
            )
        if hasattr(self, "check_out_date"):
            filters &= Q(calendar_date__end_date__gte=self.check_out_date)
        if hasattr(self, "price_gte"):
            filters &= Q(price__gte=self.price_gte)
        if hasattr(self, "price_lte"):
            filters &= Q(price__lte=self.price_lte)
        subquery = (
        ...
        return (
            queryset.annotate(min_price=Subquery(subquery))
